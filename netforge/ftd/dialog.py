...
class FtdTab(ttk.Frame):
    ...
    def _run_fdm(self, action, host, user, pw, fw):
        # FDM log lines are whole messages generated by our own client
        # (never partial device echo), so flush each one immediately -
        # the serial holdback would otherwise sit on the tail of a line
        # like "FDM login OK" until the next message arrives, which
        # during a firmware upload can be many minutes later.
        client = FdmClient(host, username=user, password=pw,
                           log=lambda msg: self._log(msg, final=True))
